...
const Video = (props) => {
  ...
  return <video className='video-element m-2' playsInline ref={ref} autoPlay />
}

const Message = ({ message: { dir, message, time, name } }) => {
  if (dir === 'center') {
    return (
    ...
    return (
      <div className='d-flex justify-content-end'>
        <div className='message text-break mb-3 p-3'>
          ...
          <div className='d-flex justify-content-end'>
            <div className='fw-italic time'> {time} </div>
          </div>
        </div>
      </div>
    )
...
const Room = (props) => {
  ...
  const [video, setVideo] = useState(
    () => sessionStorage.getItem('video') === 'true'
  )
  ...
  const socketFunctions = useCallback(async () => {
    try {
        ...
        <main className='col-7 col-sm-8 p-0 room-videos'>
          ...
          <nav className='navbar justify-content-center p-3 controls'>
            <div className='control-block d-flex'>
              ...
              >
                <i
                  className={`bi bi-camera-video${video ? '' : '-off'}-fill`}
                />
              </div>
              ...
              <div
                onClick={handleLeave}
                className='control-button text-danger p-2'
                data-for='tool-tip'
                data-tip='Leave'
              >
        ...
        <div className='col-5 col-sm-4 p-0 room-chat'>
          ...
          <div className='chat-window overflow-auto text-break'>
            <div className='messages'>
              {chat.map((cur) => {
                return <Message key={cur.id} message={cur} />
              })}
            </div>
          </div>
